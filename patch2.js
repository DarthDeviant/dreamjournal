/* ═══════════════════════════════════════════════════════════════
   patch2.js  —  Dream Journal extensions
...
function waitFor(fn, ms=60, limit=40){
  return new Promise((res,rej)=>{
    let i=0;
    const t=setInterval(()=>{
      const v=fn(); if(v){clearInterval(t);res(v);}
...
/* ════════════════════════════════════════════
   PATCH renderDecrypted
════════════════════════════════════════════ */
function patchRenderDecrypted(){
  const orig=window.renderDecrypted;
  window.renderDecrypted=function(e, animate){
    orig(e, animate);
    const applyColors=()=>{
      const cEl=$p(`edc-${e.id}`);
      const tEl=$p(`edt-${e.id}`);
      if(!cEl) return;
      const c=window.S.cache.get(e.id);
      if(!c) return;
      ...
      if(e.edited_at && tEl){
        ...
        badge.title='Last edited: '+new Date(e.edited_at).toLocaleString();
        badge.textContent='edited';
        tEl.appendChild(badge);
      }
    };
    animate ? setTimeout(applyColors,2200) : applyColors();
  };
}
...
function closeTerminal(){
  $p('p2-terminal')?.classList.remove('show');
}
...
async function commitEdit(ctx, changes){
  ...
  try {
    ...
    window.renderList();
    if(S.selId===entry.id) window.renderDecrypted(updated,false);
...
/* swap the keydown handler on #trm-input */
...
/* ── inject >_ button into statusbar ── */
...
async function p2init(){
  try { await waitFor(()=>window.S && window.renderDecrypted); }
  catch(e){ console.warn('patch2: app not ready'); return; }

  injectStatsButton();
  buildStatsPanel();
  injectTerminalButton();
  patchRenderDecrypted();
